...
from .output import Colors, debug_print, print_error, print_message
...
def is_bot_operational() -> bool:
    ...
    result = run_command(
        ["docker", "exec", container_id, "pgrep", "-f", "python.*src[/.]bot"],
    )
    ...
    if any(
        ...
        print_error(
            "Telegram API conflict detected - another bot is running with the same token",
        )
        print_message(
            "You will need to stop the other bot instance to use this one properly.",
            Colors.YELLOW,
        )
    ...
    if api_calls >= 2:
        print_message(
            f"Bot is operational ({api_calls} successful API calls detected)",
            Colors.GREEN,
        )
        return True
...
def check_bot_status() -> Dict[str, Any]:
    ...
    print_message(
        "Step 2: Waiting for container initialization (5 seconds)...",
        Colors.YELLOW,
    )
    import time
    ...
    for attempt in range(1, max_attempts + 1):
        print_message(
            f"Checking bot health (attempt {attempt}/{max_attempts})...",
            Colors.YELLOW,
        )
        ...
        if attempt == max_attempts:
            print_message(
                "Bot health check did not pass within timeout, but bot might still be functioning.",
                Colors.YELLOW,
            )
            ...
            print_message(
                "Bot health check not yet passing, waiting...",
                Colors.YELLOW,
            )
            time.sleep(5)
            continue
    ...
    if is_bot_operational():
        ...
        print_message(
            "Bot is running but might not be fully operational.",
            Colors.YELLOW,
        )
