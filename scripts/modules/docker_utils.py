...
def start_docker_macos() -> bool:
    ...
    if os.path.exists(docker_app_path):
        ...
        for attempt in range(1, max_attempts + 1):
            print_message(f"Waiting for Docker to start (attempt {attempt}/{max_attempts})...", Colors.YELLOW)
        ...
        print_message("Please start Docker for Mac manually and try again.", Colors.YELLOW)
        return False
    else:
...
def start_docker_linux() -> bool:
    ...
    print_message("Please start Docker daemon manually: sudo systemctl start docker", Colors.YELLOW)
    return False
...
def check_docker_buildx() -> bool:
    ...
    if result.returncode != 0:
        ...
        print_message("For better performance, consider installing Docker Buildx:", Colors.YELLOW)
        print_message("https://docs.docker.com/go/buildx/", Colors.YELLOW)
        return False
    return True

def run_docker_command(cmd: List[str], description: str = "", check: bool = True) -> bool:
