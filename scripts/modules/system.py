...
def setup_permissions() -> bool:
    ...
    try:
        with ErrorContext("Permission setup"):
            ...
            data_files = ["data/users.json", "data/quotes.json", "data/status.json"]
...
def check_permissions() -> bool:
    ...
    for file_name in important_files:
        ...
        if file_path.exists():
            ...
            if file_name == ".env" and os.access(
                file_path,
                os.R_OK,
                effective_ids=True,
            ):
...
def create_systemd_service() -> bool:
    ...
    try:
        with ErrorContext("Systemd service creation"):
            ...
            service_name = get_system_name() or "telegram-bot"
...
"""
            ...
            if not os.access("/etc/systemd/system/", os.W_OK):
                print_message(
                    f"Creating service file at: {service_file}",
                    Colors.YELLOW,
                )
                ...
                print_message(
                    f"To install, run: sudo mv {temp_file} {service_file}",
                    Colors.YELLOW,
                )
                print_message(
                    f"Then run: sudo systemctl enable {service_name}",
                    Colors.YELLOW,
                )
                return True
...
def cleanup_old_files(days: int = 7) -> bool:
    ...
    try:
        with ErrorContext("File cleanup"):
            ...
            if cleaned_files:
                print_message(
                    f"🧹 Cleaned up {len(cleaned_files)} old files",
                    Colors.GREEN,
                )
...
def install_dependencies() -> bool:
    ...
    if platform.system() == "Linux":
        ...
        try:
            ...
            result = subprocess.run(["which", "apt"], check=False, capture_output=True)
            if result.returncode == 0:
        ...
        try:
            ...
            result = subprocess.run(["which", "yum"], check=False, capture_output=True)
            if result.returncode == 0:
