...
from .config import (
    START_DATE, NOVOSIBIRSK_TZ,
    NOTIFICATION_DAY, NOTIFICATION_HOUR, NOTIFICATION_MINUTE,
    STATUS_MESSAGE
)
...
class StatusManager:
    ...
    def get_status_info(self, user_id: str = "global") -> str:
        ...
        if now.day <= NOTIFICATION_DAY:
            ...
            next_date = now.replace(day=NOTIFICATION_DAY, hour=NOTIFICATION_HOUR, 
                                  minute=NOTIFICATION_MINUTE, second=0, microsecond=0)
        else:
