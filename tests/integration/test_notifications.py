    ...
    def acquire(self):
        ...
        if os.path.exists(self.lock_file):
            ...
            if current_time - file_time < self.timeout_seconds:
                logger.warning(
                    f"Test lock file exists and is less than {self.timeout_seconds//60} minutes old"
                )
                return False
            else:
...
async def test_notification_settings():
    ...
    try:
        ...
        logger.info(
            f"Current time in Novosibirsk: {now.strftime('%Y-%m-%d %H:%M:%S %Z%z')}"
        )
        ...
        logger.info(
            f"✅ Next notification will be sent: {next_run.strftime('%Y-%m-%d %H:%M:%S %z')}"
        )
        ...
        logger.info(
            f"✅ This is {days_diff:.1f} days ({hours_diff:.1f} hours) from current time"
        )
        logger.info(
            f"✅ Notification time is set to {scheduler_day}rd of each month at "
...
async def test_notification_sending(bot_token):
    ...
    try:
        ...
        logger.info(
            f"✅ Test notification sent successfully to {success_count}/{len(admin_users)} admins"
        )
        return True
...
async def send_test_results(bot_token, test_success):
    ...
    try:
        ...
        logger.info(
            f"✅ Test results sent to {success_count}/{len(admin_users)} admins"
        )
        return True
...
async def main():
    ...
    if not test_lock.acquire():
        logger.warning(
            "Test is already running. Exiting to avoid duplicate notifications."
        )
        return 0
